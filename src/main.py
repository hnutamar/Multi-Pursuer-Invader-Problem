...
def main():
    ...
    #config
    if _3d:
        sc = Sim3DConfig(dt=0.02, purs_num=20, inv_num=7, obstacle=True, obstacle_rad=[3.0, 4.0], obstacle_pos=[np.array([13.0, 13.0, 6.0]), np.array([17.0, 6.0, 3.0])])
    else:
    ...
    world = SimulationWorld(sc, _3d=_3d, purs_acc=np.full(30, 5.0), inv_acc=np.full(30, 2.5), prime_acc=1.3, purs_speed=np.full(30, 8.0), inv_speed=np.full(30, 5.0), prime_speed=3.5, pursue_model=None, def_model=None, not_testing=True)
    #visualization
